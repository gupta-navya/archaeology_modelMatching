...
class EditMatchingWindow(QDialog,OpenImageMixin,PlyWindowMixin):
    ...
    def highlight_selected_model(self, frame_id: int):
        """高亮显示选中的模型"""
        # 重置所有框架样式
        for i in range(1, 5):
            frame = getattr(self, f"check_model{i}", None)
            if frame:
                frame.setStyleSheet("border: 1px solid gray;")

        # 高亮当前框架
        selected_frame = getattr(self, f"validate_match_frame{frame_id}", None)
        if selected_frame:
            selected_frame.setStyleSheet("border: 2px solid blue;")
